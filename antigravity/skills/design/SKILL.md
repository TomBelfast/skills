...
Then load `resources/design-reference.md` and check the tech stack conflicts table. Name the single CSS strategy before writing the first component.
...
Before submitting, reload `resources/design-reference.md` and check the common traps list. Then ask the user to open the result in a browser and confirm it looks right. Code that looks correct in your head but wrong in the browser is not done.
