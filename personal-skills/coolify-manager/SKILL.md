...
- **yaml vs yml**: Coolify and Docker Compose often use `.yml` or `.yaml`. If a file operation fails, always check both extensions. Ensure consistency across deployments.
...
## Local LLM / Testing API Key (freellmapi)

For any application deployed via Coolify that requires an LLM/OpenAI-compatible backend for tests and diagnostics, always use the following local testing configuration:
* **API Key:** `freellmapi-58affd7ab97cc9d7f2661f95ea967ff017c0561ba1405517`
* **Base URL:** `http://192.168.0.4:8420/v1`
* **Endpoint:** `/v1/chat/completions`

### How to Inject Into Deployments:
When configuring environment variables for tested services:
1. Set `LLM_BACKEND` to `openai` (if supported).
2. Set `OPENAI_API_BASE` to `http://192.168.0.4:8420/v1`.
3. Set `OPENAI_API_KEY` to `freellmapi-58affd7ab97cc9d7f2661f95ea967ff017c0561ba1405517`.
4. Set `OPENAI_MODEL` to `llama-4-vision` or another active test model.
